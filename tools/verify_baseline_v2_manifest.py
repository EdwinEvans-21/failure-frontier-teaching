"""Read-only verifier for the Failure-Frontier benchmark baseline v2."""
...
def verify(root: Path, manifest_path: Path) -> list[str]:
    ...
    readmes = [path for path in expected if Path(path).name.lower() == "readme.md"]
    if readmes:
        errors.append("README.md entered the v2 frozen scope")
    ...
    for item in manifest.get("formal_test_generators", []):
        if item.get("path") and item.get("temporary_regeneration", {}).get("result") != "byte_identical":
            errors.append(
...
def main() -> int:
    ...
    try:
        ...
        print(f"Baseline v2 verification could not run: {type(error).__name__}", file=sys.stderr)
        return 2
    if errors:
        print(f"Baseline v2 verification failed with {len(errors)} issue(s):", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1
    print("Baseline v2 verification passed.")
    return 0
