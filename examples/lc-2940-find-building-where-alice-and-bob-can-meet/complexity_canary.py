...
def max_task_assign_reference(tasks: list[int], workers: list[int], pills: int, strength: int) -> int:
    tasks = sorted(tasks)
...
def max_task_assign_bruteforce(tasks: list[int], workers: list[int], pills: int, strength: int) -> int:
...
def leftmost_building_reference(heights: list[int], queries: list[list[int]]) -> list[int]:
    ...
    def first(node: int, left: int, right: int, start: int, threshold: int) -> int:
        ...
        if left == right:
            return left
        mid = (left + right) // 2
        result = first(node * 2, left, mid, start, threshold)
    ...
    for a, b in queries:
        ...
        if a == b or heights[a] < heights[b]:
            ...
            answer.append(first(1, 0, n - 1, b + 1, heights[a]))
    return answer
...
def result_array_reference(nums: list[int]) -> list[int]:
    ...
    fa = Fenwick(len(keys))
    fb = Fenwick(len(keys))
    fa.add(bisect_left(keys, nums[0]) + 1, 1)
    fb.add(bisect_left(keys, nums[1]) + 1, 1)
    for x in nums[2:]:
...
def result_array_bruteforce(nums: list[int]) -> list[int]:
    ...
    b = [nums[1]]
    for x in nums[2:]:
        ga = sum((y > x for y in a))
        gb = sum((y > x for y in b))
...
def minimum_replacement_bruteforce(nums: list[int]) -> int:
    ...
    def solve(index: int, bound: int) -> int:
        ...
        for parts in range(1, value + 1):
            if (value + parts - 1) // parts <= bound:
                best = min(best, parts - 1 + solve(index - 1, value // parts))
        return best
    return solve(len(nums) - 2, nums[-1])

def minimum_manhattan_reference(points: list[list[int]]) -> int:
    transforms = [sorted(((x + y, i) for i, (x, y) in enumerate(points))), sorted(((x - y, i) for i, (x, y) in enumerate(points)))]
    answer = inf
    for removed in range(len(points)):
        worst = 0
        for values in transforms:
            lo = values[1][0] if values[0][1] == removed else values[0][0]
            hi = values[-2][0] if values[-1][1] == removed else values[-1][0]
    ...
    def count(x: int) -> int:
        ...
        for mask in range(1, 1 << len(coins)):
            ...
            for i, c in enumerate(coins):
                if mask >> i & 1:
                    multiple = multiple // gcd(multiple, c) * c
...
def minimum_one_bit_bruteforce(n: int) -> int:
    size = max(2, 1 << max(1, n.bit_length()))
    dist = {0: 0}
    queue = deque([0])
    while queue:
        x = queue.popleft()
        ...
        for i in range(1, size.bit_length()):
            if x & 1 << i - 1 and x & (1 << i - 1) - 1 == 0:
                candidates.append(x ^ 1 << i)
        for y in candidates:
            if 0 <= y < size and y not in dist:
                dist[y] = dist[x] + 1
                queue.append(y)
        ...
        for va in (0, 1):
            for vb in (0, 1):
                for actual, cost in ((op, 0), ('|' if op == '&' else '&', 1)):
                    out = va & vb if actual == '&' else va | vb
                    result[out] = min(result[out], a[va] + b[vb] + cost)
        return tuple(result)

    def reduce():
        b = values.pop()
        a = values.pop()
        values.append(combine(a, b, ops.pop()))
    ...
    def evaluate(text: str) -> int:
        ...
        def parse(index: int) -> tuple[int, int]:
            ...
            while index < len(text) and text[index] != ')':
                ...
                index += 1
                if text[index] == '(':
                    other, index = parse(index + 1)
                    index += 1
                else:
                    other = int(text[index])
                    index += 1
                value = value & other if op == '&' else value | other
            return (value, index)
        return parse(0)[0]
    target = 1 - evaluate(expression)
    queue = deque([(expression, 0)])
    seen = {expression}
    while queue:
        text, distance = queue.popleft()
        if evaluate(text) == target:
            return distance
...
def largest_special_reference(s: str) -> str:
    ...
    for i, ch in enumerate(s):
        ...
        if balance == 0:
            parts.append('1' + largest_special_reference(s[start + 1:i]) + '0')
            start = i + 1
...
class Solution:
    def leftmostBuildingQueries(self, *args, **kwargs):
        return leftmost_building_bruteforce(*args, **kwargs)
