...
def minimum_value_sum_bruteforce(nums: list[int], targets: list[int]) -> int:
    n, m = len(nums), len(targets); answer = inf
    for cuts in combinations(range(1, n), m - 1):
        starts = (0,) + cuts; ends = cuts + (n,); cost = 0; valid = True
        for start, end, target in zip(starts, ends, targets):
            value = nums[start]
            for x in nums[start + 1:end]: value &= x
            if value != target: valid = False; break
            cost += nums[end - 1]
        if valid: answer = min(answer, cost)
    return -1 if answer == inf else answer


def maximum_strength_reference(nums: list[int], k: int) -> int:
    neg = -10**60; outside = [neg] * (k + 1); inside = [neg] * (k + 1)
    outside[0] = 0
    for value in nums:
        ...
        for j in range(1, k + 1):
            ...
            inside[j] = max(
                old_in[j], old_out[j - 1], old_in[j - 1]
            ) + weight * value
...
def maximum_strength_bruteforce(nums: list[int], k: int) -> int:
    ...
    def solve(start: int, j: int) -> int:
        if j > k: return 0
        best = -10**60; weight = (k - j + 1) * (1 if j & 1 else -1)
...
def minimum_finish_time_reference(tires: list[list[int]], change: int,
                                  laps: int) -> int:
    best = [inf] * (laps + 1)
    for first, ratio in tires:
        total = 0; current = first
        for length in range(1, laps + 1):
            total += current; best[length] = min(best[length], total)
            current *= ratio
            if current > first + change: break
    dp = [inf] * (laps + 1); dp[0] = -change
...
def minimum_finish_time_bruteforce(tires: list[list[int]], change: int,
                                   laps: int) -> int:
    @lru_cache(None)
    def solve(done: int, tire: int, consecutive: int) -> int:
        if done == laps: return 0
        best = inf
        if tire >= 0:
            f, r = tires[tire]
...
def minimum_time_reference(a: list[int], b: list[int], x: int) -> int:
    n = len(a); order = sorted(zip(b, a)); dp = [0] * (n + 1)
    ...
    for time in range(n + 1):
        if initial_sum + growth_sum * time - dp[time] <= x: return time
    return -1
    ...
    for time in range(n + 1):
        ...
        for chosen in combinations(range(n), time):
            for order in __import__('itertools').permutations(chosen):
                reduction = sum(a[i] + b[i] * (step + 1)
                                for step, i in enumerate(order))
                best = max(best, reduction)
        if sum(a) + sum(b) * time - best <= x: return time
    return -1


def beautiful_partitions_reference(s: str, k: int, minimum: int) -> int:
    prime = set("2357"); n = len(s)
    if s[0] not in prime or s[-1] in prime: return 0
    start_ok = [False] * (n + 1); end_ok = [False] * (n + 1)
    start_ok[0] = True; end_ok[n] = True
    for i in range(1, n):
        boundary = s[i - 1] not in prime and s[i] in prime
        start_ok[i] = boundary; end_ok[i] = boundary
    dp = [0] * (n + 1); dp[0] = 1
    for _ in range(k):
        nxt = [0] * (n + 1); running = 0
        for end in range(1, n + 1):
            ...
            if end_ok[end]: nxt[end] = running
        dp = nxt
    return dp[n]


def beautiful_partitions_bruteforce(s: str, k: int, minimum: int) -> int:
    prime = set("2357"); answer = 0
    for cuts in combinations(range(1, len(s)), k - 1):
        starts = (0,) + cuts; ends = cuts + (len(s),)
        if all(end - start >= minimum and s[start] in prime and s[end - 1] not in prime
               for start, end in zip(starts, ends)): answer += 1
    return answer % MOD
...
def count_integers_reference(low: str, high: str, minimum: int, maximum: int) -> int:
    def upto(bound: str) -> int:
        ...
        def dp(index: int, total: int, tight: bool) -> int:
            ...
            if index == len(bound): return int(minimum <= total <= maximum)
            limit = int(bound[index]) if tight else 9
            return sum(dp(index + 1, total + digit,
                          tight and digit == limit) for digit in range(limit + 1)) % MOD
        return dp(0, 0, True)
    return (upto(high) - upto(str(int(low) - 1))) % MOD


def count_integers_bruteforce(low: str, high: str, minimum: int, maximum: int) -> int:
    return sum(minimum <= sum(map(int, str(x))) <= maximum
               for x in range(int(low), int(high) + 1)) % MOD


def maximum_nondecreasing_length_reference(nums: list[int]) -> int:
    prefix = [0]
    for value in nums: prefix.append(prefix[-1] + value)
    n = len(nums); dp = [0] * (n + 1); last = [0] * (n + 1)
    for i in range(n + 1):
        if i and dp[i - 1] > dp[i]:
            dp[i] = dp[i - 1]; last[i] = last[i - 1] + nums[i - 1]
        target = prefix[i] + last[i]
        j = bisect_left(prefix, target, i + 1)
        if j <= n and (dp[i] + 1 > dp[j] or
                       dp[i] + 1 == dp[j] and prefix[j] - prefix[i] < last[j]):
            dp[j] = dp[i] + 1; last[j] = prefix[j] - prefix[i]
    return dp[n]
...
def maximum_nondecreasing_length_bruteforce(nums: list[int]) -> int:
    ...
    for mask in range(1 << (n - 1)):
        sums = []; current = nums[0]
        for i in range(n - 1):
            if mask >> i & 1: sums.append(current); current = nums[i + 1]
            else: current += nums[i + 1]
        sums.append(current)
        if all(a <= b for a, b in zip(sums, sums[1:])): best = max(best, len(sums))
    return best
...
def max_balanced_sum_reference(nums: list[int]) -> int:
    ...
    def query(i: int) -> int:
        answer = -10**30
        while i: answer = max(answer, tree[i]); i -= i & -i
        return answer
    def update(i: int, value: int) -> None:
        while i < len(tree): tree[i] = max(tree[i], value); i += i & -i
    answer = -10**30
    for i, value in enumerate(nums):
        index = bisect_left(keys, value - i) + 1
        current = value + max(0, query(index)); update(index, current); answer = max(answer, current)
    return answer


def max_balanced_sum_bruteforce(nums: list[int]) -> int:
    answer = -10**30; n = len(nums)
...
def minimum_difference_reference(nums: list[int]) -> int:
    half = len(nums) // 2; left, right = nums[:half], nums[half:]
    a = [[] for _ in range(half + 1)]; b = [[] for _ in range(half + 1)]
    for mask in range(1 << half):
        count = mask.bit_count(); a[count].append(sum(left[i] for i in range(half) if mask >> i & 1)); b[count].append(sum(right[i] for i in range(half) if mask >> i & 1))
    total = sum(nums); answer = inf
    for count in range(half + 1):
        values = sorted(b[half - count])
        for x in a[count]:
            target = total / 2 - x; pos = bisect_left(values, target)
            for j in (pos - 1, pos):
                if 0 <= j < len(values): answer = min(answer, abs(total - 2 * (x + values[j])))
    return int(answer)


def minimum_difference_bruteforce(nums: list[int]) -> int:
    half = len(nums) // 2; total = sum(nums)
    return min(abs(total - 2 * sum(nums[i] for i in chosen))
               for chosen in combinations(range(len(nums)), half))


def minimum_cut_cost_reference(n: int, cuts: list[int]) -> int:
    points = [0] + sorted(cuts) + [n]; size = len(points)
    dp = [[0] * size for _ in range(size)]
    for gap in range(2, size):
        for left in range(size - gap):
            right = left + gap
            dp[left][right] = points[right] - points[left] + min(
...
class Solution:
    def minimumFinishTime(self, tires, changeTime, numLaps):
        answer = minimum_finish_time_reference(tires, changeTime, numLaps)
        return [] if isinstance(answer, list) else ("" if isinstance(answer, str) else 0)
