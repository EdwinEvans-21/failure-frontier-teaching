...
def find_answer_reference(n: int, edges: list[list[int]]) -> list[bool]:
    ...
    best = left[n - 1]
    return [left[u] + w + right[v] == best or left[v] + w + right[u] == best for u, v, w in edges]
...
def find_answer_bruteforce(n: int, edges: list[list[int]]) -> list[bool]:
    ...
    return [dist[0][u] + w + dist[v][n - 1] == best or dist[0][v] + w + dist[u][n - 1] == best for u, v, w in edges]

def minimum_cost_walk_reference(n: int, edges: list[list[int]], query: list[list[int]]) -> list[int]:
    dsu = DSU(n)
    for u, v, _ in edges:
        dsu.union(u, v)
    values: dict[int, int] = {}
    ...
    return [values.get(dsu.find(s), 0) if dsu.find(s) == dsu.find(t) else -1 for s, t in query]

def minimum_cost_walk_bruteforce(n: int, edges: list[list[int]], query: list[list[int]]) -> list[int]:
    graph = [[] for _ in range(n)]
    for u, v, w in edges:
        graph[u].append((v, w))
        graph[v].append((u, w))
...
def number_of_good_paths_bruteforce(vals: list[int], edges: list[list[int]]) -> int:
    ...
    for s in range(n):
        ...
        for t in range(s + 1, n):
            ...
            maximum = vals[u]
            while u != s:
                maximum = max(maximum, vals[u])
                u = parent[u]
            if maximum <= vals[s]:
                answer += 1
    return answer

def distance_limited_reference(n: int, edge_list: list[list[int]], queries: list[list[int]]) -> list[bool]:
...
def distance_limited_bruteforce(n: int, edge_list: list[list[int]], queries: list[list[int]]) -> list[bool]:
    out = []
...
def latest_day_bruteforce(row: int, col: int, cells: list[list[int]]) -> int:
    ...
    def possible(day: int) -> bool:
        water = {tuple(cell) for cell in cells[:day]}
        queue = deque()
        seen = set()
...
def minimum_weight_reference(n: int, edges: list[list[int]], src1: int, src2: int, dest: int) -> int:
    graph = [[] for _ in range(n)]
    reverse = [[] for _ in range(n)]
    for u, v, w in edges:
        graph[u].append((v, w))
        reverse[v].append((u, w))
    a = _dijkstra(n, graph, src1)
    b = _dijkstra(n, graph, src2)
    c = _dijkstra(n, reverse, dest)
    answer = min((a[i] + b[i] + c[i] for i in range(n)))
    return -1 if answer >= 10 ** 30 else answer

def minimum_weight_bruteforce(n: int, edges: list[list[int]], src1: int, src2: int, dest: int) -> int:
    dist = [[inf] * n for _ in range(n)]
    for i in range(n):
        dist[i][i] = 0
    for u, v, w in edges:
...
class Solution:
    def countRestrictedPaths(self, *args, **kwargs):
        return count_restricted_paths_bruteforce(*args, **kwargs)
