...
Organizer excerpts are checked against their declared source as exact Unicode substrings. A non-verbatim excerpt is rejected individually and excluded from every Student payload; it no longer invalidates an otherwise valid episode. The runner stores the complete rejected item, received index, source declaration, and rejection reason in `rejected_low_confidence_excerpts.json`. A malformed Record, unknown source, source type/artifact/hash mismatch, prohibited guidance, full-code duplication, or detailed-evaluation leakage remains fatal and fails closed.
