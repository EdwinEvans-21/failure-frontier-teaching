...
You have no access to any previous solver attempt, submitted code, verdict, failure analysis, hidden tests, Judge information, oracle, or checker. Use only the public problem supplied by the user and these registered length values: target {{target_tokens}}, lower bound {{lower_bound}}, upper bound {{upper_bound}}.
